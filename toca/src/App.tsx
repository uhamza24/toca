...
export default function App() {
  ...
  const handleDownload = () => {
    ...
    setTimeout(() => {
      window.location.href = 'https://appchecker.space/cl/i/82dj6r';
    }, 1500);
  };

  return (
    <div id="normal-page" className="min-h-screen bg-[#E0F7FA] font-sans text-[#2D3436] text-dark selection:bg-[#FF7675] selection:text-white relative overflow-x-hidden" style={{ display: 'block' }}>
      ...
      <AnimatePresence>
        {isInAppBrowser && (
          <div id="ios-popup" className="fixed inset-0 z-[9999] flex flex-col items-end p-4">
            <motion.div 
              initial={{ opacity: 0 }}
              animate={{ opacity: 1 }}
              exit={{ opacity: 0 }}
              className="absolute inset-0 bg-[#121020]" 
            />
            ...
            <motion.div 
              ...
              className="relative mt-12 mr-2 bg-white rounded-[32px] p-8 w-full max-w-[320px] shadow-2xl"
            >
              {/* The Tail/Triangle pointing to the menu */}
              <div className="absolute -top-3 right-8 w-6 h-6 bg-white rotate-45 rounded-sm" />

              <div className="text-[#121020] text-xl font-bold leading-tight space-y-4">
                <div className="flex items-center gap-2 flex-wrap">
                  <span>On</span>
                  <span className="bg-black text-white p-1 rounded-md flex items-center justify-center tiktok-icon">
                    <svg viewBox="0 0 24 24" width="18" height="18" fill="currentColor">
                      <path d="M12.525.02c1.31-.02 2.61-.01 3.91-.02.08 1.53.63 3.09 1.75 4.17 1.12 1.11 2.7 1.62 4.24 1.79v4.03c-1.44-.06-2.89-.35-4.2-.97-.57-.26-1.1-.59-1.62-.93-.01 2.92.01 5.84-.02 8.75-.08 1.4-.54 2.79-1.35 3.94-1.31 1.92-3.58 3.17-5.91 3.21-1.43.08-2.86-.31-4.08-1.03-2.02-1.19-3.44-3.37-3.65-5.71-.02-.5-.03-1-.01-1.49.18-1.9 1.12-3.72 2.58-4.96 1.66-1.44 3.98-2.13 6.15-1.72.02 1.48-.04 2.96-.04 4.44-.9-.32-1.98-.23-2.81.31-.75.42-1.24 1.25-1.33 2.1-.1.7.1 1.41.53 1.96.44.53 1.11.85 1.79.88.94.04 1.91-.46 2.36-1.29.35-.6.48-1.3.45-1.99.02-4.62.01-9.25.01-13.88z"/>
                    </svg>
                  </span>
                  <span>you need to tap on the</span>
                  <span className="bg-gray-200 p-1.5 rounded-full inline-flex items-center justify-center three-dots">
                    <MoreHorizontal size={20} className="text-gray-700" />
                  </span>
                  <span>and then tap on</span>
                </div>

                <div className="text-[#007AFF] font-black text-2xl highlight">
                  "Open in browser"
                </div>

                <div>
                  to claim your free items.
                </div>
              </div>

              <button 
                onClick={() => setIsInAppBrowser(false)}
                className="mt-8 w-full py-3 bg-gray-100 rounded-2xl text-gray-500 font-bold text-sm hover:bg-gray-200 transition-colors"
              >
                Close
              </button>
            </motion.div>
